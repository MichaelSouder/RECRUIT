...
# Tags used after load (match scripts/airgap/bundle.py's tag_if_untagged fallback)
...
MANIFEST="$OUT_DIR/MANIFEST.txt"
{
  ...
  echo "  airgap-cli (+ airgap/ package)"
...
README_AIRGAP="$OUT_DIR/README-AIR-GAP.txt"
{
  ...
  echo "Load + start (requires python3, stdlib only -- no pip install needed):"
  echo "  ./airgap-cli update-containers ."
  echo "Or, to also bring up Postgres/Redis on a completely fresh host:"
  echo "  copy recruit-airgap.env.example to recruit-airgap.env, edit, then:"
  echo "  ./airgap-cli stack-up ."
  echo "Or from repo: ./scripts/airgap-cli update-containers /path/to/bundle"
} > "$README_AIRGAP"

echo "==> Copy air-gap documentation and helper scripts into bundle"
cp "$ROOT/docs/AIRGAP_DEPLOY.md" "$OUT_DIR/AIRGAP_DEPLOY.md"
cp "$ROOT/scripts/airgap-cli" "$OUT_DIR/airgap-cli"
cp -R "$ROOT/scripts/airgap" "$OUT_DIR/airgap"
rm -rf "$OUT_DIR/airgap/tests" "$OUT_DIR/airgap/requirements-dev.txt" "$OUT_DIR/airgap/__pycache__"
cp "$ROOT/scripts/recruit-airgap.env.example" "$OUT_DIR/recruit-airgap.env.example"
chmod +x "$OUT_DIR/airgap-cli"
